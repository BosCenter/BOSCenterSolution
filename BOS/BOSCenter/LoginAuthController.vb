...
Public Class LoginAuthController
    ...
    Public Function GetIPAddress() As String
        ...
        End Try
    End Function
    Dim LoginTransation As LoginMDL = New LoginMDL
    Dim Trans_LoginData As Trans_Login = New Trans_Login
    ...
    'Save Order For BOS
    Public Function GetValue(ByVal Companycode As String, ByVal LoginID As String, ByVal UserPassword As String, ByVal BillAmount As String, ByVal Order_ID As String) As Trans_Login
        Dim Trans_Login As New Dictionary(Of String, String) From {
             {"Status", ""},
             {"CompanyCode", ""},
             {"LoginID", ""},
             {"AgentID", ""},
        ...
        Trans_LoginData.LoginID = LoginID
        Dim result As String = ""

        Dim ConvertResponse As String = ""

        If Companycode.Trim = "" Then
            result = "Please Enter CompanyCode."
            Trans_LoginData.Status = result
            'Return JsonConvert.SerializeObject(returnPair, Formatting.None)
            Return Trans_LoginData
            Exit Function
        End If
        ...
        Trans_LoginData.Status = Companycode.Trim

        Dim localDS As New DataSet
        If Len(LoginID.Trim) = 10 And IsNumeric(LoginID.Trim) Then
            'Case If user Is Customer
            localDS = OpenDsWithSelectQuery("select * from " & DBName.Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration where   MobileNo='" & LoginID & "' and AgentType='Customer' and AgentPassword='" & GV.EncryptString(GV.key, UserPassword) & "' and ActiveStatus='Active' ")
        ElseIf LoginID.Trim.Contains("-") Then
            localDS = OpenDsWithSelectQuery("select * from " & DBName.Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration where   RegistrationId='" & LoginID & "' and AgentPassword='" & GV.EncryptString(GV.key, UserPassword) & "'  and AgentType in ('Customer','Retailer')  and ActiveStatus='Active' ")
        Else
            ...
            If localDS.Tables(0).Rows.Count > 0 Then
                ...
                Else
                    ...
                    If executeDMLQuery(bosqry) > 0 Then
                        result = "Success"
                        Trans_LoginData.Status = result
                        Trans_LoginData.AgentID = localDS.Tables(0).Rows(0).Item("RegistrationId")
                        Trans_LoginData.Order_ID = Order_ID
                        Trans_LoginData.Pay_Trans_ID = Pay_Trans_ID
                    Else
                        result = "Failed"
                        Trans_LoginData.Status = result
                        ''Return JsonConvert.SerializeObject(returnPair, Formatting.None)
    ...
    'Dim VStatus, VCompanyName, VCompanyCode, VLoginID, VAgentID, VAgentName, VAgentType, VWalletBalance, VServiceType, VServiceCharges As String

    Public Function GetValue(ByVal Companycode As String, ByVal LoginID As String, ByVal UserPassword As String) As LoginMDL
        ...
        Dim result As String = ""
        Dim rescompanyname As String = ""
        Dim rescompanycode As String = ""
        Dim resloginid As String = ""
        Dim resagentid As String = ""
        Dim resagentname As String = ""
        Dim resAgentType As String = ""
        Dim resWalletBalance As String = ""
        Dim resServiceType As String = ""
        Dim resServiceCharges As String = ""
        Dim LoginData As LoginMDL = New LoginMDL
        ...
        LoginData.Status = Companycode.Trim
        LoginData.CompanyName = AddInVar("CompanyName", " BosCenter_DB.dbo.BOS_ClientRegistration where CompanyCode='" & Companycode.Trim & "' and [Status]='Active' ")
        ...
        If localDS.Tables.Count > 0 Then
            If localDS.Tables(0).Rows.Count > 0 Then
                ...
                result = "Success"
                returnPair("Status") = result
                'LoginData.CompanyCode = localDS.Tables(0).Rows(0).Item("CompanyCode")
                LoginData.Status = "Success"


                LoginData.AgentID = localDS.Tables(0).Rows(0).Item("RegistrationId")
                LoginData.LoginID = LoginID
                LoginData.AgentName = localDS.Tables(0).Rows(0).Item("FirstName") & " " & localDS.Tables(0).Rows(0).Item("LastName")
                LoginData.AgentType = localDS.Tables(0).Rows(0).Item("AgentType")
                LoginData.AgentType = localDS.Tables(0).Rows(0).Item("AgentType")
                LoginData.WalletBalance = returnWalletBalCalculation(localDS.Tables(0).Rows(0).Item("RegistrationId"), DBName)
                LoginData.ServiceType = AddInVar("ServiceType", "" & DBName.Trim & ".[dbo].[BOS_ProductServiceMaster] where Title='Flight' ")
                LoginData.ServiceCharges = AddInVar("ServiceCharge", "" & DBName.Trim & ".[dbo].[BOS_ProductServiceMaster] where Title='Flight' ") '
