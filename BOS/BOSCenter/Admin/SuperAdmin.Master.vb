...
Public Class SuperAdmin
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            lblgroupType.Font.Size = FontUnit.Larger
            ...
            If UserGroup.Trim.ToUpper = "Admin".Trim.ToUpper Or UserGroup.Trim.ToUpper = "Super Admin".Trim.ToUpper Then
                theme = GV.FL.AddInVar("isnull(ChangeTheme,'')", lbl_DBName.Text.Trim & ".dbo.CRM_Login_Details where User_ID='" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "'")
                If theme = "" Then
                    lnkred.Attributes("href") = "~/css/admin_Blue.css"
                Else
                    lnkred.Attributes("href") = theme
                End If
            Else
                theme = GV.FL.AddInVar("isnull(ChangeTheme,'')", lbl_DBName.Text.Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration where RegistrationId='" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "'")
                If theme = "" Then
                    lnkred.Attributes("href") = "~/css/admin_Blue.css"
                Else
                    lnkred.Attributes("href") = theme
            ...
            If Not IsPostBack = True Then
                ...
                Dim baseurl As String = ("http://ip-api.com/json/103.77.42.184")
                ...
                Page.Title = GV.FL.AddInVar("CompanyName", " " & GV.DefaultDatabase.Trim & ".dbo.BOS_ClientRegistration where DatabaseName='" & dbName & "'")
    ...
    Public Sub Calculation()
        Try
            ...
            ElseIf group.Trim.ToUpper = "Admin".Trim.ToUpper Then
                ...
                'Querystring = Querystring & " + (select isnull(sum (TransferAmt),0) from " & lbl_DBName.Text.Trim & ".dbo.BOS_TransferAmountToAgents where Amount_Type in ('Commission','Service Charge','Withdraw','Commission-Refund','Service Charge-Refund') and TransferTo='Admin') "
                'Querystring = Querystring & " + (select isnull(sum (TransferAmt),0) from " & lbl_DBName.Text.Trim & ".dbo.BOS_TransferAmountToAgents where Amount_Type in ('Deposit') and TransferFrom='Super Admin' and TransferTo='Admin')"
                'Querystring = Querystring & " - (select isnull(sum (TransferAmt),0) from " & lbl_DBName.Text.Trim & ".dbo.BOS_TransferAmountToAgents where Amount_Type in ('Service Charge') and TransferFrom='Admin' and 	TransferTo='Super Admin' )"
                'Querystring = Querystring & " - (select isnull(sum (TransferAmt),0) from " & lbl_DBName.Text.Trim & ".dbo.BOS_TransferAmountToAgents where Amount_Type in ('Withdraw') and TransferFrom='Admin' and 	TransferTo='Super Admin' )"
                'Querystring = Querystring & " - ( select isnull(sum (TransferAmt),0) from " & lbl_DBName.Text.Trim & ".dbo.BOS_TransferAmountToAgents where Amount_Type in ('Commission','Service Charge','Transaction Charge') and TransferFrom='Admin')"
                'Querystring = Querystring & " - ( select isnull(sum (TransferAmt),0) from " & lbl_DBName.Text.Trim & ".dbo.BOS_TransferAmountToAgents where Amount_Type in ('MakePayment','Deposit','Service Charge-Refund','Commission-Refund') and TransferFrom='Admin')"
                'Querystring = Querystring & " from " & lbl_DBName.Text.Trim & ".dbo.BOS_TransferAmountToAgents where Amount_Type in ('MakePayment') and TransferFrom='Super Admin' and 	TransferTo='Admin'"
                ...
                '            Dim xx As String = ds.Tables(0).Rows(0).Item(0)
                '            If xx.Trim = "" Then
                '                xx = "0"
                '            End If
                '            lblWalletBalance.Text = xx
                '        End If
                '    End If
                'End If


                lblWalletBalance.Text = GV.returnAdminWalletBalance(GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim)
                If Not IsNumeric(lblWalletBalance.Text) Then
                    lblWalletBalance.Text = "0"
                End If

                lblMainBal.Text = lblWalletBalance.Text
    ...
    Protected Sub btnChangeTheme1_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnChangeTheme1.Click
        Try
            ...
            Dim UserGroup As String = GV.get_SuperAdmin_SessionVariables("Group", Request, Response)
            If UserGroup.Trim.ToUpper = "Admin".Trim.ToUpper Or UserGroup.Trim.ToUpper = "Super Admin".Trim.ToUpper Then
                GV.FL.DMLQueries("update " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.CRM_Login_Details set ChangeTheme='" & theme & "' where User_ID='" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "'")
            Else
                GV.FL.DMLQueries("update " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration set ChangeTheme='" & theme & "' where RegistrationId='" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "'")
            End If
        Catch ex As Exception
    ...
    Protected Sub btnChangeTheme6_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnChangeTheme6.Click
        Try

            lnkred.Attributes("href") = "~/css/admin_kv.css"
            theme = "~/css/admin_kv.css"
            Dim UserGroup As String = GV.get_SuperAdmin_SessionVariables("Group", Request, Response)
            If UserGroup.Trim.ToUpper = "Admin".Trim.ToUpper Or UserGroup.Trim.ToUpper = "Super Admin".Trim.ToUpper Then
                GV.FL.DMLQueries("update " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.CRM_Login_Details set ChangeTheme='" & theme & "' where User_ID='" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "'")
            Else
