...
Public Class BOS_Card_Kyc_Report_RT
    ...
    Public Sub Bind()
        Try
            ...
            ElseIf ddlSelectCriteria.SelectedValue.Trim.ToUpper = "Request ID".Trim.ToUpper Then
                Filter = "  RequestID  like '" & GV.parseString(txtSearchingValue.Text.Trim) & "%'"
            ElseIf ddlSelectCriteria.SelectedValue.Trim.ToUpper = "Registration ID".Trim.ToUpper Then
                Filter = "  KCode like '" & GV.parseString(txtSearchingValue.Text.Trim) & "%'"
            ElseIf ddlSelectCriteria.SelectedValue.Trim.ToUpper = "First Name".Trim.ToUpper Then 'ReferenceNumber
                Filter = "  FirstName like '" & GV.parseString(txtSearchingValue.Text.Trim) & "%'"
            ElseIf ddlSelectCriteria.SelectedValue.Trim.ToUpper = "Reference Number".Trim.ToUpper Then 'ReferenceNumber
                Filter = "  ReferenceNumber like '" & GV.parseString(txtSearchingValue.Text.Trim) & "%'"
            End If
            ...
            If chkduration.Checked = True Then
                If Status.Trim = "" And condition.Trim = "" And Filter.Trim = "" And GROUPFilter.Trim = "" Then
                    durationDate = " where RequestDate between  '" & GV.FL.returnDateMonthWise(txtFrom.Text) & "' and '" & GV.FL.returnDateMonthWise(txtTO.Text) & "'"
                Else
            ...
            If GV.get_SuperAdmin_SessionVariables("Group", Request, Response).Trim.ToUpper = "Distributor".Trim.ToUpper Or GV.get_SuperAdmin_SessionVariables("Group", Request, Response).Trim.ToUpper = "Retailer".Trim.ToUpper Or GV.get_SuperAdmin_SessionVariables("Group", Request, Response).Trim.ToUpper = "Master Distributor".Trim.ToUpper Or GV.get_SuperAdmin_SessionVariables("Group", Request, Response).Trim.ToUpper = "Customer".Trim.ToUpper Then
                'dis,sd,re,cust
                Querystring = "select RID as SrNo,RequestID,KCode as 'RegistrationId',(CONVERT(VARCHAR(11),RequestDate,106)) as 'RequestDate', (isnull(Salutation,'') +' '+ isnull(FirstName,'') +' '+ isnull(MiddleName,'') +' '+ isnull(LastName,'')) as 'Name',Address,PhoneNo,AadharNumber,CardNumber,ReferenceNumber as 'RefNo',UploadAddharCard_Back,UploadAddharCard_Front,UploadPhoto ,CompanyCode  from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Card_Kyc_RT_Details   " & Status & " " & condition & "  " & Filter & " " & GROUPFilter & " " & durationDate & " order by rid desc"
            ElseIf GV.get_SuperAdmin_SessionVariables("Group", Request, Response).Trim.ToUpper = "Super Admin".Trim.ToUpper Then
                Querystring = "select RID as SrNo,RequestID,KCode as 'RegistrationId',(CONVERT(VARCHAR(11),RequestDate,106)) as 'RequestDate', (isnull(Salutation,'') +' '+ isnull(FirstName,'') +' '+ isnull(MiddleName,'') +' '+ isnull(LastName,'')) as 'Name',Address,PhoneNo,AadharNumber,CardNumber ,ReferenceNumber as 'RefNo',UploadAddharCard_Back,UploadAddharCard_Front,UploadPhoto,CompanyCode  from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Card_Kyc_RT_Details " & Status & " " & condition & "  " & Filter & " " & GROUPFilter & " " & durationDate & " order by rid desc"
                'super admin
            Else
            ...
            If Not Querystring = "" Then
                ...
                If GridView1.Rows.Count > 0 Then
                    ...
                    For i As Integer = 0 To GridView1.Rows.Count - 1
                        Dim lnk_AF_dwnload As LinkButton = DirectCast(GridView1.Rows(i).FindControl("lnk_AF_dwnload"), LinkButton)
                        Dim lbl_AF_Attachment As Label = DirectCast(GridView1.Rows(i).FindControl("lbl_AF_Attachment"), Label)
                        If lbl_AF_Attachment.Text = "" Then
                            lnk_AF_dwnload.Visible = False
                        Else
                            lnk_AF_dwnload.Visible = True
                        End If

                        Dim lnk_AB_dwnload As LinkButton = DirectCast(GridView1.Rows(i).FindControl("lnk_AB_dwnload"), LinkButton)
                        Dim lbl_AB_Attachment As Label = DirectCast(GridView1.Rows(i).FindControl("lbl_AB_Attachment"), Label)
                        If lbl_AB_Attachment.Text = "" Then
                            lnk_AB_dwnload.Visible = False
                        Else
                            lnk_AB_dwnload.Visible = True
                        End If

                        Dim lnk_PH_dwnload As LinkButton = DirectCast(GridView1.Rows(i).FindControl("lnk_PH_dwnload"), LinkButton)
                        Dim lbl_PH_Attachment As Label = DirectCast(GridView1.Rows(i).FindControl("lbl_PH_Attachment"), Label)
                        If lbl_PH_Attachment.Text = "" Then
                            lnk_PH_dwnload.Visible = False
                        Else
                            lnk_PH_dwnload.Visible = True
                        End If
    ...
    Protected Sub UpdateDetails_Click(ByVal sender As Object, ByVal e As EventArgs)
        Try
            ...
            Dim ReqID As String = GV.parseString(GridView1.Rows(gvrow.RowIndex).Cells(2).Text)

            Response.Redirect("cKycFormRT_Print.aspx?ReqID=" & ReqID.Trim)
    ...
    Protected Sub DownloadRow_click_AF(ByVal sender As Object, ByVal e As EventArgs)
        Try
            Dim btndetails As LinkButton = TryCast(sender, LinkButton)
            Dim gvrow As GridViewRow = DirectCast(btndetails.NamingContainer, GridViewRow)
            Dim lblAttachment As Label = DirectCast(GridView1.Rows(gvrow.RowIndex).FindControl("lbl_AF_Attachment"), Label)
            DownloadDoc(lblAttachment.Text)
        Catch ex As Exception
        End Try
    End Sub
    Protected Sub DownloadRow_click_AB(ByVal sender As Object, ByVal e As EventArgs)
        Try
            Dim btndetails As LinkButton = TryCast(sender, LinkButton)
            Dim gvrow As GridViewRow = DirectCast(btndetails.NamingContainer, GridViewRow)
            Dim lblAttachment As Label = DirectCast(GridView1.Rows(gvrow.RowIndex).FindControl("lbl_AB_Attachment"), Label)
            DownloadDoc(lblAttachment.Text)
        Catch ex As Exception
        End Try
    End Sub

    Protected Sub DownloadRow_click_PH(ByVal sender As Object, ByVal e As EventArgs)
        Try
            Dim btndetails As LinkButton = TryCast(sender, LinkButton)
            Dim gvrow As GridViewRow = DirectCast(btndetails.NamingContainer, GridViewRow)
            Dim lblAttachment As Label = DirectCast(GridView1.Rows(gvrow.RowIndex).FindControl("lbl_PH_Attachment"), Label)
            DownloadDoc(lblAttachment.Text)
        Catch ex As Exception
        End Try
    End Sub
        ...
        Try
            If Not IsPostBack Then
                ...
                If grp.Trim.ToUpper = "Distributor".Trim.ToUpper Or grp.Trim.ToUpper = "Master Distributor".Trim.ToUpper Or grp.Trim.ToUpper = "Retailer".Trim.ToUpper Or grp.Trim.ToUpper = "Customer".Trim.ToUpper Then
                    ddlSelectCriteria.Items.Remove("Registration ID")
                    Exit Sub
    ...
    Protected Sub ImagebtnExcel_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImagebtnExcel.Click
        Try
            GV.ExportToExcel(GridView1, Response, "CardKYCReport")
        Catch ex As Exception
        End Try
    ...
    Protected Sub ImagebtnPdf_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImagebtnPdf.Click
        Try
            GV.ExportToPdf(GridView1, Response, "CardKYCReport")
        Catch ex As Exception
        End Try
    End Sub
    ...
    Protected Sub Imagebtnword_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImagebtnWOrd.Click
        Try
            GV.ExportToWord(GridView1, Response, "CardKYCReport")
        Catch ex As Exception
        End Try
    End Sub
