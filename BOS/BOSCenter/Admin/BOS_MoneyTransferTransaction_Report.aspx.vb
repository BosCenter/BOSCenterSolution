        ...
        Try
            ...
            Else
                ...
                Else
                    SearchColumnName = ""
                End If
                Querystring = " Select RID as 'Sr No',(CONVERT(VARCHAR(11),TransferDate,106)) as 'Transfer Date',CONVERT(varchar(15),CAST(RecordDateTime AS TIME),100) as Time,OrderNo as 'UTR No', (select top 1 AgencyName from BOS_Dis_SubDis_Retailer_Registration where RegistrationId=fr.RefrenceNo)  as 'Agency Name',TranscationId as 'Transcation ID',Receipent as 'Beneficiary Name',AccountNo,MobileNo as 'Sender Mobile No',Amount,BankName as 'Bank Name',Method,Process,APIMessage as 'Message' from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_MoneyTransfer_API fr  where (APIMessage in ('Success','Transaction successful')  or APIMessage like '%Transaction Successful%' ) and RefrenceNo='" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "'  " & SearchColumnName & "  order by  RID Desc "
