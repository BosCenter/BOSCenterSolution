Public Class BOS_MoneyTransferTransaction_Report
    ...
    Public Sub Bind()
        Try
            ...
            If chkduration.Checked = True Then
                ...
                Querystring = " Select RID as SrNo,(CONVERT(VARCHAR(11),TransferDate,106)) as TransferDate,CONVERT(varchar(15),CAST(RecordDateTime AS TIME),100) as Time,OrderNo,RefrenceNo,TranscationId,CustomerID,MobileNo,Amount,BankName,Method,Process,APIMessage as 'Message' from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_MoneyTransfer_API  b where TransferDate between  '" & GV.FL.returnDateMonthWise(txtFrom.Text) & "' and '" & GV.FL.returnDateMonthWise(txtTO.Text) & "'  and RefrenceNo='" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "' " & SearchColumnName & "    order by  RID Desc"
            ...
            Else
                ...
                Else
                    SearchColumnName = ""
                End If
                Querystring = " Select RID as SrNo,(CONVERT(VARCHAR(11),TransferDate,106)) as TransferDate,CONVERT(varchar(15),CAST(RecordDateTime AS TIME),100) as Time,OrderNo,RefrenceNo,TranscationId,CustomerID,MobileNo,Amount,BankName,Method,Process,APIMessage as 'Message' from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_MoneyTransfer_API  where RefrenceNo='" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "'  " & SearchColumnName & "  order by  RID Desc "
