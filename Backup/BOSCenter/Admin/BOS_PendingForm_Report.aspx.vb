...
Public Class BOS_PendingForm_Report
    ...
    Public Sub Bind()
        Try
            ...
            If grp.Trim.ToUpper = "Distributor".Trim.ToUpper Or grp.Trim.ToUpper = "Master Distributor".Trim.ToUpper Or grp.Trim.ToUpper = "Retailer".Trim.ToUpper Then
                Querystring = "select RID as SrNo,(CONVERT(VARCHAR(11),ComplaintDate,106)) as ComplaintDate,ComplaintID,kCode,kCodeType,Product,(Problem+'/'+Complaint) as ComplaintProblem,isnull(Attachment,'') as Attachment,ComplaintStatus,AllotedTo,(CONVERT(VARCHAR(11),AllotedDateTime,106)) as AllotedDateTime,Remarks,ClosedBy,(CONVERT(VARCHAR(11),AllotedDateTime,106)) as ClosedDate from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Complaint_Master where ComplaintStatus='Pending' " & GROUPFilter & " " & Filter & " order by RID Desc"
            Else
