Public Class BOS_ProductVsProblem_SA
    ...
    Public Sub Bind()
        Try
            ds = GV.FL.OpenDsWithSelectQuery("select RID as SrNo,* from " & GV.DefaultDatabase.Trim & ".dbo.BOS_ComplaintVSProblem_Master_SA order by RID desc  ")
    ...
    Private Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            If lblSessionFlag.Text = 0 Then
                ...
                If GV.FL.RecCount(" " & GV.DefaultDatabase.Trim & ".dbo.BOS_ComplaintVSProblem_Master_SA where Product='" & VProduct & "' and ProductProblem='" & VProblem & "' ") > 0 Then 'Change where condition according to Criteria 
                    lblError.Text = "Record Already Exists."
                    lblError.CssClass = "errorlabels"
                Else

                    QryStr = "insert into " & GV.DefaultDatabase.Trim & ".dbo.BOS_ComplaintVSProblem_Master_SA (Product,ProductProblem,UpdatedBy,UpdatedOn) values('" & VProduct & "','" & VProblem & "','" & VUpdatedBy & "'," & VUpdatedOn & " );"
            ...
            ElseIf lblSessionFlag.Text = 1 Then
                If GV.FL.RecCount(" " & GV.DefaultDatabase.Trim & ".dbo.BOS_ComplaintVSProblem_Master_SA where (ProductProblem='" & VProblem & "' and not ProductProblem ='" & lblUpadate.Text & "') and (Product='" & VProduct & "' and not Product ='" & lblproduct.Text & "')") > 0 Then 'Change where condition according to Criteria 
                    lblError.Text = "Record Already Exists."
                    lblError.CssClass = "errorlabels"
                Else
                    QryStr = "update  " & GV.DefaultDatabase.Trim & ".dbo.BOS_ComplaintVSProblem_Master_SA set Product='" & VProduct & "', ProductProblem='" & VProblem & "', UpdatedBy='" & VUpdatedBy & "', UpdatedOn=" & VUpdatedOn & " where RID=" & lblRID.Text.Trim & " ;"
                    If GV.FL.DMLQueries(QryStr) = True Then
                        lblSessionFlag.Text = 0
                        Clear()
                        Bind()
    ...
    Protected Sub btnDeleteRow_Click(ByVal sender As Object, ByVal e As EventArgs)
        Try
            If Not lblRID.Text = "" Then
                QryStr = "delete from " & GV.DefaultDatabase.Trim & ".dbo.BOS_ComplaintVSProblem_Master_SA where RID=" & lblRID.Text & ""
                If GV.FL.DMLQueries(QryStr) = True Then
                    Bind()
                    Clear()
    ...
    Protected Sub ImagebtnExcel_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles ImagebtnExcel.Click
        Try
            If GridView1.Rows.Count > 0 Then
                GV.ExportToExcel_New(GridView1, Response, "", "ProductVsProblem", "select * from " & GV.DefaultDatabase.Trim & ".dbo.BOS_ComplaintVSProblem_Master_SA order by rid desc ", "dyanamic")
            End If
        Catch ex As Exception
    ...
    Protected Sub ImagebtnWOrd_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles ImagebtnWOrd.Click
        Try
            ...
            If GridView1.Rows.Count > 0 Then
                GV.ExportToWord_New(GridView1, Response, "ProductVsProblem", "select * from " & GV.DefaultDatabase.Trim & ".dbo.BOS_ComplaintVSProblem_Master_SA  order by rid desc ", "dyanamic")
            End If
    ...
    Protected Sub Imagepdf_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles Imagepdf.Click
        Try
            If GridView1.Rows.Count > 0 Then
                GV.ExportToPdf_New(GridView1, "", Response, "ProductVsProblem", "select * from " & GV.DefaultDatabase.Trim & ".dbo.BOS_ComplaintVSProblem_Master_SA  order by rid desc ", "dyanamic")
            End If
        Catch ex As Exception
