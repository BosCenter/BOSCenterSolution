...
Public Class GlobalVariable
    ...
    Public Sub New(ByVal ConnectionFrom As String)
        Try
            ...
            If ConnectionFrom.Trim.ToUpper = "ADMIN" Then
                FL = New EmvFuncLibForWebVBNET.WebVBDOTNET(get_SuperAdmin_SessionVariables("DataBaseName", HttpContext.Current.Request, HttpContext.Current.Response).Trim, "S", "103.205.66.210,2499", "CapUY77RawrIZa7h", "sa")
                FL_AdminLogin = New EmvFuncLibForWebVBNET.WebVBDOTNET("BosCenter_DB", "S", "103.205.66.210,2499", "CapUY77RawrIZa7h", "sa")
            ElseIf ConnectionFrom.Trim.ToUpper = "SUPERADMIN" Then
                FL = New EmvFuncLibForWebVBNET.WebVBDOTNET("BosCenter_DB", "S", "103.205.66.210,2499", "CapUY77RawrIZa7h", "sa")
                FL_AdminLogin = New EmvFuncLibForWebVBNET.WebVBDOTNET("BosCenter_DB", "S", "103.205.66.210,2499", "CapUY77RawrIZa7h", "sa")
            ElseIf ConnectionFrom.Trim.ToUpper = "Webservice".Trim.ToUpper Then
                FL_WebService = New EmvFuncLibForWebVBNET.WebVBDOTNET("BosCenter_DB", "S", "103.205.66.210,2499", "CapUY77RawrIZa7h", "sa")
            End If
    ...
    Public Function returnAPIBalance(DBName As String) As Long
        ...
        Try
            ...
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where  TransferFrom='Super Admin' and TransferTo='Admin' and Amount_Type in ('Deposit','MakePayment')  ) "
            Querystring = Querystring & " +"
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where  TransferFrom='Admin' and Amount_Type='BILLPAY-Refund')"
            Querystring = Querystring & " +"
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where  TransferFrom='Super Admin' and TransferTo='Admin' and Amount_Type='Commission')   "
            Querystring = Querystring & " +"
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where  TransferFrom='Admin' and Amount_Type='RECH-Refund')   "
            Querystring = Querystring & " +"
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where  TransferFrom='Super Admin' and TransferTo='Admin' and  Amount_Type='Service Charge-Refund')   "
            Querystring = Querystring & " )"
            Querystring = Querystring & " - "
            Querystring = Querystring & " ("
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where  TransferFrom='Admin' and TransferTo='Super Admin' and Amount_Type='Withdraw') "
            Querystring = Querystring & " +"
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where  TransferTo='Admin' and Amount_Type='BILLPAY')   "
            Querystring = Querystring & " +"
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where TransferFrom='Admin' and TransferTo='Super Admin' and  Amount_Type='Commission-Refund')   "
            Querystring = Querystring & " +"
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where  TransferTo='Admin' and Amount_Type in ('Money Transfer','Money Transfer-2'))   "
            Querystring = Querystring & " +"
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where  TransferTo='Admin' and Amount_Type='PAN CARD')   "
            Querystring = Querystring & " +"
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where  TransferTo='Admin' and Amount_Type='RECH')   "
            Querystring = Querystring & " +"
            Querystring = Querystring & " (select isnull(sum(isnull(TransferAmt,0)),0) from " & DBName.Trim & ".dbo.BOS_TransferAmountToAgents where TransferFrom='Admin' and TransferTo='Super Admin' and  Amount_Type='Service Charge')   "
            Querystring = Querystring & " )"
