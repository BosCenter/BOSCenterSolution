...
Public Class BOS_Payout_Transfer_Report
    ...
    Public Sub Bind()
        Try
            ...
            If chkduration.Checked = True Then
                ...
                Querystring = " Select RID as 'Sr No',utr as 'UTR No.',trxn_id as 'Transaction Id',beneficiaryAccNo as 'Account No.',beneficiaryName as 'Name',amount as' Amount',beneDescription as 'Description',banestatus as 'Status',creationDateTime as 'DateTime'  from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.FidyPayoutResponse where banestatus='Success' " & SearchColumnName & "  and LoginID='" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "' and CompanyCode='" & GV.get_SuperAdmin_SessionVariables("CompanyCode", Request, Response) & "' and  creationDateTime between '" & CDate(txtFrom.Text).ToString("yyyy-MM-dd") & " 00:00:00.000' and '" & CDate(txtTO.Text).ToString("yyyy-MM-dd") & " 23:59:59.999'  order by rid desc"
            ...
            Else
                ...
                Else
                    SearchColumnName = ""
                End If
                'Querystring = " Select RID as 'Sr No',(CONVERT(VARCHAR(11),TransactionDate,106)) as 'Transfer Date',CONVERT(varchar(15),CAST(RecordDateTime AS TIME),100) as Time ,Amt_Transfer_TransID as 'Transcation ID',Ref_TransID as Reference Id,TransferFrom as 'Transfer From', TransferTo as'Transfer To',TransferAmt as 'Transfer Amount', Remark,TransferFromMsg,TransferToMsg from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_TransferAmountToAgents where Amount_Type='Debit' and TransferFrom= '" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "'  " & SearchColumnName & "  order by  RID Desc "
                Querystring = " Select RID as 'Sr No',utr as 'UTR No.',trxn_id as 'Transaction Id',beneficiaryAccNo as 'Account No.',beneficiaryName as 'Name',amount as' Amount',beneDescription as 'Description',banestatus as 'Status',creationDateTime as 'DateTime'  from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.FidyPayoutResponse where banestatus='Success' " & SearchColumnName & " and LoginID='" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "' and CompanyCode='" & GV.get_SuperAdmin_SessionVariables("CompanyCode", Request, Response) & "'  order by  RID Desc "
