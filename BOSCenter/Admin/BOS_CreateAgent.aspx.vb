...
Public Class BOS_CreateAgent
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            Else
                If BAckPAth = "" Then
                    txtCreditBalLimit.ReadOnly = True
                    btnDelete.Visible = False
                Else
                    txtCreditBalLimit.ReadOnly = False
            ...
            If Not IsPostBack Then
                ...
                If Not Session("RecordID") = "" And Session("RecordEdit") = 1 And Session("RecordEditConfirm") = 9 Then
                    ...
                    btnClear.Text = "Close"
                    '  btnDelete.Enabled = True
                    lblformsectionhead3.Text = "Edit Create Agents "
                    DS = GV.FL.OpenDs("" & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration where RID='" & Session("RecordID") & "'")
                        ...
                        If DS.Tables.Count > 0 Then
                            ...
                            If Not IsDBNull(DS.Tables(0).Rows(0).Item("ActiveStatus")) Then
                                If Not DS.Tables(0).Rows(0).Item("ActiveStatus").ToString() = "" Then
                                    ddlStatus.SelectedValue = GV.parseString(DS.Tables(0).Rows(0).Item("ActiveStatus").ToString())
                                Else
                            ...
                            If Not IsDBNull(DS.Tables(0).Rows(0).Item("UploadSelfyWithDoc")) Then

                                If Not DS.Tables(0).Rows(0).Item("UploadSelfyWithDoc").ToString() = "" Then
                                    filePath = DS.Tables(0).Rows(0).Item("UploadSelfyWithDoc").ToString()
                                    filename = Path.GetFileName(filePath)
                                    ext = Path.GetExtension(filename)
                                    If (UCase(ext) = UCase(".jpg")) Or (UCase(ext) = UCase(".bmp")) Or (UCase(ext) = UCase(".jpeg")) Or (UCase(ext) = UCase(".png")) Or (UCase(ext) = UCase(".gif")) Then

                                        btnUpload_SelfyWithDocument.ToolTip = GV.parseString(DS.Tables(0).Rows(0).Item("UploadSelfyWithDoc").ToString())
                                        Image_SelfyWithDocument.ImageUrl = btnUpload_SelfyWithDocument.ToolTip
                                    End If
                                    btnUpload_SelfyWithDocument.Text = "Download"
                                    btnDeleteUpload_SelfyWithDocument.Enabled = True
                                Else
                                    btnUpload_SelfyWithDocument.ToolTip = ""
                                    Image_SelfyWithDocument.ImageUrl = "~/images/uploadimage.png"
                                End If
                            Else
                                btnUpload_SelfyWithDocument.ToolTip = ""
                                Image_SelfyWithDocument.ImageUrl = "~/images/uploadimage.png"
                            ...
                            If Not IsDBNull(DS.Tables(0).Rows(0).Item("UploadSignature")) Then

                                If Not DS.Tables(0).Rows(0).Item("UploadSignature").ToString() = "" Then
                                    filePath = DS.Tables(0).Rows(0).Item("UploadSignature").ToString()
                                    filename = Path.GetFileName(filePath)
                                    ext = Path.GetExtension(filename)
                                    If (UCase(ext) = UCase(".jpg")) Or (UCase(ext) = UCase(".bmp")) Or (UCase(ext) = UCase(".jpeg")) Or (UCase(ext) = UCase(".png")) Or (UCase(ext) = UCase(".gif")) Then

                                        btnUpload_UploadSignature.ToolTip = GV.parseString(DS.Tables(0).Rows(0).Item("UploadSignature").ToString())
                                        Image_UploadSignature.ImageUrl = btnUpload_UploadSignature.ToolTip
                                    End If
                                    btnUpload_UploadSignature.Text = "Download"
                                    btnDeleteUpload_UploadSignature.Enabled = True
                                Else
                                    btnUpload_UploadSignature.ToolTip = ""
                                    Image_UploadSignature.ImageUrl = "~/images/uploadimage.png"
                                End If
                            Else
                                btnUpload_UploadSignature.ToolTip = ""
                                Image_UploadSignature.ImageUrl = "~/images/uploadimage.png"
                            ...
                            If BAckPAth = "" Then
                                ...
                                btnDeleteUpload_SelfyWithDocument.Enabled = False
                                btnDeleteUpload_UploadSignature.Enabled = False
                            End If
    ...
    Private Sub btnClear_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnClear.Click
        Try
            If btnClear.Text = "Close" Then
                Response.Redirect("BOS_SearchAgent.aspx") '/Change the name of form
            Else
                Clear()
                ddlAgentType_SelectedIndexChanged(sender, e)
            End If
    ...
    Private Sub btnPopupYes_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPopupYes.Click
        Try
            If Session("Workfor") = "Delete" Then
                If btnPopupYes.Text = "Ok" Then
                    If Session("EditFlag") = 1 Then
                        Response.Redirect("BOS_SearchAgent.aspx") '// Change form Name
                    Else
                        Clear()
                        Exit Sub
                    End If
                End If
                Dim QryStr As String = ""
                QryStr = " delete from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_BankDetails where RegistrationId = '" & txtRegistrationId.Text.Trim & "';"
                QryStr = QryStr & " " & " insert into " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration_Deleted (UploadSelfyWithDoc,UploadSignature,DeletedBy,DeletedOn,AgentType,RegistrationId,RegistrationDate,AgencyName,PanCardNumber,FirstName,LastName,EmailID,MobileNo,DOB,BusinessType,AlternateMobileNo,OfficeAddress,PermanentAddress,Pincode,[State],City,AddharCardNo,GSTNO,WebSite,UploadPanCard,UploadAddharCard_Front,UploadAddharCard_Back,UploadOtherProof,UploadPhoto,UpdatedBy,UpdatedOn,RecordDateTime,AgentPassword,ChangeTheme,RefrenceID,RefrenceType,CreditBalnceLimit,TransactionPin,ActiveStatus,EmpCode,RechargeAPI_Status,PANCardAPI_Status,MoneyTransferAPI_Status,AEPS_API_Status,District,BcCode,StateID,DistrictID,CompanyCode,Ref_Code,HoldAmt,HoldAmtRemarks) select UploadSelfyWithDoc,UploadSignature,'" & GV.get_SuperAdmin_SessionVariables("LoginID", Request, Response) & "',getdate(),AgentType,RegistrationId,RegistrationDate,AgencyName,PanCardNumber,FirstName,LastName,EmailID,MobileNo,DOB,BusinessType,AlternateMobileNo,OfficeAddress,PermanentAddress,Pincode,[State],City,AddharCardNo,GSTNO,WebSite,UploadPanCard,UploadAddharCard_Front,UploadAddharCard_Back,UploadOtherProof,UploadPhoto,UpdatedBy,UpdatedOn,RecordDateTime,AgentPassword,ChangeTheme,RefrenceID,RefrenceType,CreditBalnceLimit,TransactionPin,ActiveStatus,EmpCode,RechargeAPI_Status,PANCardAPI_Status,MoneyTransferAPI_Status,AEPS_API_Status,District,BcCode,StateID,DistrictID,CompanyCode,Ref_Code,HoldAmt,HoldAmtRemarks  from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration where RID=" & lblRID.Text & "  ; "
                QryStr = QryStr & " " & "delete from " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration where RID=" & lblRID.Text.Trim & ";"
            ...
            Dim VPanCard_Path, VAddharCardBack_Path, VAddharCardFront_PAth, VPhoto_Path, VOtherDocuments_Path, VSelfyWithDocument, VUploadSignature As String
            ...
            If Not btnUpload_SelfyWithDocument.ToolTip.Trim = "" Then
                Dim fi As New FileInfo(btnUpload_SelfyWithDocument.ToolTip.Trim)
                VSelfyWithDocument = "~/DistributorDocuments/" & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & "/" & VRegistrationId & "/" & fi.Name
            Else
                VSelfyWithDocument = ""
            End If

            If Not btnUpload_UploadSignature.ToolTip.Trim = "" Then
                Dim fi As New FileInfo(btnUpload_UploadSignature.ToolTip.Trim)
                VUploadSignature = "~/DistributorDocuments/" & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & "/" & VRegistrationId & "/" & fi.Name
            Else
                VUploadSignature = ""
            End If
            ...
            If GV.get_SuperAdmin_SessionVariables("Group", Request, Response).Trim.ToUpper = "Master Distributor".Trim.ToUpper Or GV.get_SuperAdmin_SessionVariables("Group", Request, Response).Trim.ToUpper = "Distributor".Trim.ToUpper Or GV.get_SuperAdmin_SessionVariables("Group", Request, Response).Trim.ToUpper = "Retailer".Trim.ToUpper Then
            Else
                VRefrenceID = "ADMIN"
                VRefrenceType = "ADMIN"
            ...
            If Session("EditFlag") = 0 Then
                ...
                Dim QryStr As String = "insert into " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration (AEPS_Onboard_Status,UploadSelfyWithDoc,UploadSignature,Encrypted_Pass,HoldAmt,District,AEPS_API_Status,RechargeAPI_Status,MoneyTransferAPI_Status,PANCardAPI_Status,EmpCode,ActiveStatus,TransactionPin,CreditBalnceLimit,RefrenceID,RefrenceType,AgentPassword,UploadPanCard,UploadAddharCard_Front,UploadAddharCard_Back,UploadOtherProof,UploadPhoto,UpdatedBy,UpdatedOn,RecordDateTime,RegistrationDate,AgentType,AgencyName,FirstName,EmailID,DOB,AlternateMobileNo,PermanentAddress,State,AddharCardNo,WebSite,RegistrationId,PanCardNumber,MobileNo,OfficeAddress,City,LastName,Pincode,BusinessType,GSTNO) values('No', '" & VSelfyWithDocument & "','" & VUploadSignature & "','" & Encrypted_Pass & "' ,0,'" & VDistrict & "','" & APIStatus & "','" & APIStatus & "','" & APIStatus & "','" & APIStatus & "','" & EMPCode & "','" & VActiveStatus & "','" & trnsactionpinNo & "','" & VCreditBalLimit & "','" & VRefrenceID & "','" & VRefrenceType & "','" & Encrypted_Pass & "','" & VPanCard_Path & "','" & VAddharCardFront_PAth & "','" & VAddharCardBack_Path & "','" & VOtherDocuments_Path & "','" & VPhoto_Path & "','" & VUpdatedBy & "'," & VUpdatedOn & "," & VRecord_DateTime & ",'" & VRegisterDate & "', '" & VAgentType & "','" & VAgencyName & "','" & VFirstName & "','" & VEmailID & "','" & VDOB & "','" & VAlternateMobileNo & "','" & VPermanentAddress & "','" & VState & "','" & VAddharCardNo & "','" & VWebSite & "','" & VRegistrationId & "','" & VPanCardNumber & "','" & VMobileNo & "','" & VOfficeAddress & "','" & VCity & "','" & VLastName & "','" & VPincode & "','" & VBusinessType & "','" & VGSTNO & "' ) ;"
                ...
                If GV.FL.DMLQueriesBulk(QryStr) = True Then
                    ...
                    If GV.FL.AddInVar("AllowSms", GV.DefaultDatabase.Trim & ".dbo.Autonumber").ToString.Trim.ToUpper = "Yes".ToUpper Then
                        ...
                        ElseIf GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim = "CMP1218" Then
                            '  Dim vMessage As String = "Dear " & VFirstName.Trim & " " & VLastName.Trim & " Your Login id: " & VMobileNo & " Password: " & Vpassword & " Tpin: " & trnsactionpinNo & " Please Do Not Share With Anyone.Thanks for using PayKre Web: https://bit.ly/3p1OkPj App: https://bit.ly/3Szblqb"
                            Dim vMessage As String = "Welcome To Paykre, Dear Customer, Your ID Is" & VMobileNo & ",Your Password Is " & Vpassword & ",Your PIN is" & trnsactionpinNo & ",Enjoy PayKre Services And Get Cash Back/Bonus On Every Recharges & All Services.jpgrouphttps://paykre.org/"
                            GV.send_Template_Based_SMS_API(VMobileNo, vMessage, "Agent Registration", "CMP1218")
                        Else
                    ...
                    If Not btnUpload_SelfyWithDocument.ToolTip.Trim = "" Then
                        ...
                        If Not Directory.Exists(Server.MapPath(destinationpath)) Then
                            Directory.CreateDirectory(Server.MapPath(destinationpath))
                        End If
                        If File.Exists(Server.MapPath(btnUpload_SelfyWithDocument.ToolTip.Trim)) Then
                            File.Move(Server.MapPath(btnUpload_SelfyWithDocument.ToolTip.Trim), Server.MapPath(VSelfyWithDocument))
                        End If
                    End If
                    ...
                    If Not btnUpload_UploadSignature.ToolTip.Trim = "" Then
                        ...
                        If Not Directory.Exists(Server.MapPath(destinationpath)) Then
                            Directory.CreateDirectory(Server.MapPath(destinationpath))
                        End If
                        If File.Exists(Server.MapPath(btnUpload_UploadSignature.ToolTip.Trim)) Then
                            File.Move(Server.MapPath(btnUpload_UploadSignature.ToolTip.Trim), Server.MapPath(VUploadSignature))
                        End If
                    End If
            ...
            ElseIf Session("EditFlag") = 1 Then
                ...
                If GV.get_SuperAdmin_SessionVariables("Group", Request, Response).Trim.ToUpper = "Admin".Trim.ToUpper Or GV.get_SuperAdmin_SessionVariables("Group", Request, Response).Trim.ToUpper = "Super Admin".Trim.ToUpper Then
                Else
    ...
    Private Sub Clear()
        Try
            ...
            btnUpload_UploadSignature.Text = "Upload"
            btnUpload_SelfyWithDocument.Text = "Upload"
    ...
    Private Sub btnDelete_click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        Try
            If GV.FL.RecCount("" & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration where RefrenceID='" & txtRegistrationId.Text.Trim & "' ") > 0 Then
                lblDialogMsg.CssClass = ""
    ...
    Protected Sub btnUpload_UploadSignature_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpload_UploadSignature.Click
        Try
            Try
                lblErrorImage_UploadSignature.Text = ""
                lblErrorImage_UploadSignature.CssClass = ""
                If (btnUpload_UploadSignature.Text = "Download") Then
                    DownloadDoc(btnUpload_UploadSignature.ToolTip)
                Else
                    If FileUpload_UploadSignature.HasFile = True Then

                        filePath = FileUpload_UploadSignature.PostedFile.FileName
                        filename = Path.GetFileName(filePath)
                        ext = Path.GetExtension(filename)

                        If (UCase(ext) = UCase(".jpg")) Or (UCase(ext) = UCase(".bmp")) Or (UCase(ext) = UCase(".jpeg")) Or (UCase(ext) = UCase(".png")) Or (UCase(ext) = UCase(".gif")) Then

                            SaveImage(FileUpload_UploadSignature, btnUpload_UploadSignature, btnDeleteUpload_UploadSignature, "UploadSignature")
                            Dim fi As New FileInfo(btnUpload_UploadSignature.ToolTip.ToString())
                            Dim ext As String = fi.Extension.ToUpper
                            If fi.Extension.ToUpper = UCase(".jpg") Or fi.Extension.ToUpper = UCase(".bmp") Or fi.Extension.ToUpper = UCase(".jpeg") Or fi.Extension.ToUpper = UCase(".png") Or fi.Extension.ToUpper = UCase(".gif") Then
                                Image_UploadSignature.ImageUrl = btnUpload_UploadSignature.ToolTip.ToString()
                                btnUpload_UploadSignature.Focus()
                            Else
                                Image_UploadSignature.ImageUrl = "~/images/documents.png"
                                btnUpload_UploadSignature.Focus()
                            End If
                        Else
                            lblInformation.Text = "Invalid Document Type."
                    ...
                    Else
                        lblErrorImage_UploadSignature.Text = "No file Selected for Upload Signature"
                        lblErrorImage_UploadSignature.CssClass = "errorlabels"
                        btnUpload_UploadSignature.Focus()
                    End If
                End If
            Catch ex As Exception
    ...
    Protected Sub btnUpload_SelfyWithDocument_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpload_SelfyWithDocument.Click
        Try
            Try
                lblErrorImage_SelfyWithDocument.Text = ""
                lblErrorImage_SelfyWithDocument.CssClass = ""
                If (btnUpload_SelfyWithDocument.Text = "Download") Then
                    DownloadDoc(btnUpload_SelfyWithDocument.ToolTip)
                Else
                    If FileUpload_SelfyWithDocument.HasFile = True Then

                        filePath = FileUpload_SelfyWithDocument.PostedFile.FileName
                        filename = Path.GetFileName(filePath)
                        ext = Path.GetExtension(filename)

                        If (UCase(ext) = UCase(".jpg")) Or (UCase(ext) = UCase(".jpeg")) Or (UCase(ext) = UCase(".png")) Or (UCase(ext) = UCase(".gif")) Then

                            SaveImage(FileUpload_SelfyWithDocument, btnUpload_SelfyWithDocument, btnDeleteUpload_SelfyWithDocument, "SelfyWithDocument")
                            Dim fi As New FileInfo(btnUpload_SelfyWithDocument.ToolTip.ToString())
                            Dim ext As String = fi.Extension.ToUpper
                            If fi.Extension.ToUpper = UCase(".jpg") Or fi.Extension.ToUpper = UCase(".jpeg") Or fi.Extension.ToUpper = UCase(".png") Or fi.Extension.ToUpper = UCase(".gif") Then
                                Image_SelfyWithDocument.ImageUrl = btnUpload_SelfyWithDocument.ToolTip.ToString()
                                btnUpload_SelfyWithDocument.Focus()
                            Else
                                Image_SelfyWithDocument.ImageUrl = "~/images/documents.png"
                                btnUpload_SelfyWithDocument.Focus()
                            End If
                        Else
                            lblInformation.Text = "Invalid Document Type."
                    ...
                    Else
                        lblErrorImage_SelfyWithDocument.Text = "No file Selected for SelfyWithDocument"
                        lblErrorImage_SelfyWithDocument.CssClass = "errorlabels"
                        btnUpload_SelfyWithDocument.Focus()
                    End If
                End If
            Catch ex As Exception
    ...
    Protected Sub btnDeleteUpload_Signature_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDeleteUpload_OtherDocuments.Click
        Try
            ...
            lblDeleteInfo.CssClass = ""
            lblDeleteInfo.Text = "This Action will Remove the Other Document Permanently.<br/><b>Are You Sure You want To Delete ? </b>"
        ...
        End Try
    End Sub
    Protected Sub btnDeleteUpload_UploadSignature_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDeleteUpload_UploadSignature.Click
        Try
            ...
            lblDeleteInfo.CssClass = ""
            lblDeleteInfo.Text = "This Action will Remove Signature Permanently.<br/><b>Are You Sure You want To Delete ? </b>"

            lblDeleteDocumentInfo.Text = "UploadSignature"
            ModalPopupExtender4.Show()
    ...
    Protected Sub btnDeleteUpload_SelfyWithDocument_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDeleteUpload_SelfyWithDocument.Click
        Try
            ...
            lblDeleteInfo.CssClass = ""
            lblDeleteInfo.Text = "This Action will Remove SelfyWithDocument Permanently.<br/><b>Are You Sure You want To Delete ? </b>"

            lblDeleteDocumentInfo.Text = "SelfyWithDocument"
            ModalPopupExtender4.Show()
    ...
    Protected Sub btnDelete_Document_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete_Document.Click
        Try
            ...
            If btnDelete_Document.Text = "Yes" Then
                ...
                ElseIf lblDeleteDocumentInfo.Text = "UploadSignature" Then

                    Dim imgpath As String = btnUpload_UploadSignature.ToolTip
                    File.Delete(Server.MapPath(imgpath))
                    qry = "update " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration set UploadSignature=''  where RegistrationId='" & VMemberID & "' "

                    btnUpload_UploadSignature.ToolTip = ""
                    btnUpload_UploadSignature.Text = "Upload"
                    btnDeleteUpload_UploadSignature.Enabled = False
                    btnDeleteUpload_UploadSignature.Focus()
                    Image_UploadSignature.ImageUrl = "~/images/uploadimage.png"

                    lblDeleteInfo.Text = "Signature Removed Successfully."
                    lblDeleteInfo.CssClass = "successlabels"
                ElseIf lblDeleteDocumentInfo.Text = "SelfyWithDocument" Then

                    Dim imgpath As String = btnUpload_SelfyWithDocument.ToolTip
                    File.Delete(Server.MapPath(imgpath))
                    qry = "update " & GV.get_SuperAdmin_SessionVariables("DataBaseName", Request, Response).Trim & ".dbo.BOS_Dis_SubDis_Retailer_Registration set UploadSelfyWithDoc=''  where RegistrationId='" & VMemberID & "' "

                    btnUpload_SelfyWithDocument.ToolTip = ""
                    btnUpload_SelfyWithDocument.Text = "Upload"
                    btnDeleteUpload_SelfyWithDocument.Enabled = False
                    btnDeleteUpload_SelfyWithDocument.Focus()
                    Image_SelfyWithDocument.ImageUrl = "~/images/uploadimage.png"
