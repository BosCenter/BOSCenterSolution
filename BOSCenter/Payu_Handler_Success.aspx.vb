...
Public Class Payu_Handler_Success
                            ...
                            If VRequest_Purpose.Trim.ToUpper = "CC" Then
                                ...
                                If Not GV.FL.RecCount(" " & lblDBName.Text.Trim & ".dbo.BOS_TransferAmountToAgents Where Amt_Transfer_TransID='" & VAmtTransTransID & "' ") > 0 Then 'Change where condition according to Criteria 
                                    ...
                                    If GV.FL.DMLQueries(QryStr) = True Then
                                        ...
                                        Dim Service() As String = GV.FL.AddInVar("convert(nvarchar,ServiceCharge)+':'+ServiceType", "" & GV.DefaultDatabase.Trim & ".dbo.BOS_ProductServiceVsAdmin_SA where AdminID='" & lblCompanyCode.Text & "' and Title='PayuMoney' ").Split(":")
                                        ...
                                        If Not ss.Trim = "" Then
                                            If ss.Trim.Contains(":") Then
                                                ...
                                                VMobileNo = details(3)
                                                Vpassword = GV.DecryptString(GV.key, details(4))
                                        ...
                                        If GV.FL.AddInVar("AllowSms", GV.DefaultDatabase.Trim & ".dbo.Autonumber").ToString.Trim.ToUpper = "Yes".ToUpper Then
                                            ...
                                            ElseIf lblDBName.Text.Trim.ToUpper = "CMP1218" Then
                                                Dim vMessage As String = "Welcome To Paykre, Dear Customer, Your ID Is" & VMobileNo & ",Your Password Is " & Vpassword & ",Your PIN is" & VtrnsactionpinNo & ",Enjoy PayKre Services And Get Cash Back/Bonus On Every Recharges & All Services.jpgrouphttps://paykre.org/"
