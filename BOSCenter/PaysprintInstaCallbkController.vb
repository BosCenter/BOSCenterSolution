...
Public Class PaysprintInstaCallbkController
    ...
    Dim v_event, txnstatus, Message, ackno, amounts,
    ...
    Public Function logerrors(ByVal [error] As String)
        ...
        Dim filename As String = "BOSLog_" + DateTime.Now.ToString("dd-MM-yyyy-HH-mm-ss") + ".txt"
    ...
    Public Function executeDMLQuery(ByVal Query As String) As Integer   'done
        ...
        Try
            ...
            con = New SqlConnection("Server=103.35.121.85,5022;DataBase=BosCenter_DB;user id=sa;password=Boscenter@123;Connection Timeout=30;Connection Lifetime=0;Min Pool Size=0;Max Pool Size=600;Pooling=true".ToString())
    ...
    End Function
    'Public Function StrConv(ByVal str As String) As String   
    '    If str = "&nbsp;" Or str = "" Then
    '        Return ""
    '    Else
    ...
    Public Function PostData(<FromBody()> ByVal data As JObject) As ResponseData
        Try
            ...
            amounts = json1.SelectToken("param")("amount")
            ...
            If aaEvent = "UPIQR_SUCCESS" Then
                ...
                Dim QryStr As String = " insert into " & dbName & ".dbo.BOS_TransferAmountToAgents (Ref_TransID,TransIpAddress,API_TransId,Amt_Transfer_TransID,TransferToMsg,TransferFromMsg,Amount_Type,Remark,TransactionDate,TransferFrom,TransferTo,TransferAmt,RecordDateTime,UpdatedBy,UpdatedOn,data_remitter_full_name,customer_virtual_address) values( '" & v_Ref_TransID.Trim & "','" & GetIPAddress() & "','" & Trns_Id & "','" & Trns_Id & "','Your Wallet is Credited by BOS CENTER PVT LTD - Account Deposit','Your Wallet is Debited by Admin  (" & v_AdminID & ")  - Payin','Deposit','" & "Add By Wallet - Account Deposit" & "',getdate(),'Super Admin','Admin','" & VTransferAmt & "',getdate(),'Admin',getdate(),'" & customer_account_name & "','" & customer_virtual_address & "' ) ;"
                ...
                QryStr = QryStr & " insert into " & dbName & ".dbo.BOS_TransferAmountToAgents(Ref_TransID, TransIpAddress, API_TransId, Amt_Transfer_TransID, TransferToMsg, TransferFromMsg, Amount_Type, Remark, TransactionDate, TransferFrom, TransferTo, TransferAmt, RecordDateTime, UpdatedBy, UpdatedOn,data_remitter_full_name,customer_virtual_address) values( '" & v_Ref_TransID.Trim & "','" & GetIPAddress() & "','" & VAmtTransTransID & "','" & refid & "','" & VTransferToMsg & "','" & VTransferFromMsg & "','" & AmouontType & "','" & "Add By Wallet - Payin" & "',getdate(),'" & VTransferFrom & "','" & VTransferTo & "','" & VTransferAmt & "',getdate(),'Admin',getdate() ,'" & customer_account_name & "','" & customer_virtual_address & "' ) ;"
                ...
                If v_AgentType = "Retailer" Then
                    Service1 = AddInVar("convert(nvarchar,ServiceCharge)+':'+ServiceType", dbName & ".dbo.BOS_ProductServiceMaster where Title='UPI PayIn' and service_charge_type='Retailer'").Split(":")
                ElseIf v_AgentType = "Customer" Then
                    Service1 = AddInVar("convert(nvarchar,ServiceCharge)+':'+ServiceType", dbName & ".dbo.BOS_ProductServiceMaster where Title='UPI PayIn' and service_charge_type='Customer'").Split(":")
                End If

                'After Discussion with Sir
                If VTransferAmt > 250 Then
                    If Service1.Length > 1 Then
                        If Service1(1).Trim = "Percentage" Then
                            NetAmount1 = (CDec(VTransferAmt) * CDec((Service1(0)))) / 100
                            GstAmt = ((NetAmount1 * 18) / 100)
                            NetAmount1 = NetAmount1 + GstAmt
                ...
                If NetAmount1 > 0 Then
                    ...
                    Dim VFrom As String = "Your Account is debited by ServiceCharge " & NetAmount1 & " Rs. inculding GST Due to Add Money - " & RTE & " ."
                    Dim VTo As String = "Your Account is credited by ServiceCharge " & NetAmount1 & " Rs. Due to Add Money - " & RTE & " ."
                    QryStr = "insert into " & dbName & ".dbo.BOS_TransferAmountToAgents (TransIpAddress,API_TransId,Actual_Transaction_Amount,Ref_TransID,TransferToMsg,TransferFromMsg,Amount_Type,Remark,TransactionDate,TransferFrom,TransferTo,TransferAmt,RecordDateTime,UpdatedBy,UpdatedOn,data_remitter_full_name,customer_virtual_address,GSTAmt) values( '" & GetIPAddress() & "','" & v_Transfer_Trans_ID & "','" & VTransferAmt & "', '" & GV.parseString(refid) & "','" & VTo & "','" & VFrom & "','Service Charge','Service Charge','" & Now.ToString("MM/dd/yyyy") & "','" & RTE & "','ADMIN','" & NetAmount1 & "',getdate(),'Admin',getdate() ,'" & customer_account_name & "','" & customer_virtual_address & "','" & GstAmt & "' ) ;"
                    executeDMLQuery(QryStr)
                End If
                ...
                'After Discussion with Sir
                If VTransferAmt > 250 Then
                    Dim Service() As String = AddInVar("convert(nvarchar,ServiceCharge)+':'+ServiceType", "" & dbName & ".dbo.BOS_ProductServiceVsAdmin_SA where AdminID='" & v_AdminID & "' and Title='UPI PayIn' ").Split(":")
                ...
                If NetAmount > 0 Then
                    Dim RTE As String = v_AgentID
                    Dim VFrom As String = "Your Account is debited by ServiceCharge " & NetAmount & " Rs. inculding GST Due to Add Money - " & RTE & " ."
                    Dim VTo As String = "Your Account is credited by ServiceCharge " & NetAmount & " Rs. Due to Add Money - " & RTE & " ."
                    QryStr = "insert into " & dbName & ".dbo.BOS_TransferAmountToAgents (TransIpAddress,API_TransId,Actual_Transaction_Amount,Ref_TransID,TransferToMsg,TransferFromMsg,Amount_Type,Remark,TransactionDate,TransferFrom,TransferTo,TransferAmt,RecordDateTime,UpdatedBy,UpdatedOn,data_remitter_full_name,customer_virtual_address,GSTAmt) values( '" & GetIPAddress() & "','" & v_Transfer_Trans_ID & "','" & VTransferAmt & "', '" & v_Transfer_Trans_ID & "','" & VTo & "','" & VFrom & "','Service Charge','Service Charge','" & Now.ToString("MM/dd/yyyy") & "','ADMIN','SUPER ADMIN','" & NetAmount & "',getdate(),'Admin',getdate() ,'" & customer_account_name & "','" & customer_virtual_address & "','" & GstAmt & "' ) ;"
                    executeDMLQuery(QryStr)
                End If
            ...
            Try
                If v_AgentID = "RTE-554" Or v_AgentID = "RTE-809" Then
                    Dim Objdata = Encoding.UTF8.GetBytes(json1.ToString)
                    ServicePointManager.Expect100Continue = True
                    ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12
                    Dim result_post = SendRequest("https://payma.globalvipgame.com/bos/payment/notify", Objdata, "application/json", "POST")
                End If
            Catch ex As Exception
    ...
    Public Function Commision_Calculation_For_SuperAdmin(Amount As Decimal, ByVal OperatorCategory As String, ByVal OperatorCode As String, APIName As String) As String
        ...
        Try
            ...
            Dim qryStr As String = "select * from " & "BosCenter_DB" & ".dbo.BOS_ProductServiceMaster_SA where Title='" & APIName.Trim & "' and ActiveStatus='Active'"
            ds = New DataSet
            ds = OpenDsWithSelectQuery(qryStr)
                ...
                If ds.Tables.Count > 0 Then
                    If ds.Tables(0).Rows.Count > 0 Then
                        ...
                        If VContainCategory.Trim.ToUpper = "YES" And VSlabApplicable.Trim.ToUpper = "Without Slab".ToUpper Then
                            ...
                            qryStr = "select * from " & "BosCenter_DB" & ".dbo.BOS_OperatorWiseCommission_SA where ActiveStatus='Active' and APIName='" & APIName.Trim & "' and Category='" & OperatorCategory & "' and 	Code='" & OperatorCode & "'"
                            ds = New DataSet
                            ds = OpenDsWithSelectQuery(qryStr)
                            If Not ds Is Nothing Then
                                If ds.Tables.Count > 0 Then
                                    If ds.Tables(0).Rows.Count > 0 Then
                                        ...
                                        If Not IsDBNull(ds.Tables(0).Rows(0).Item("SA_CommissionType")) Then
                                            If Not ds.Tables(0).Rows(0).Item("SA_CommissionType").ToString() = "" Then
                                                V_CommissionType = (ds.Tables(0).Rows(0).Item("SA_CommissionType").ToString())
                                            End If
                                        End If

                                        If Not IsDBNull(ds.Tables(0).Rows(0).Item("SA_Commission")) Then
                                            If Not ds.Tables(0).Rows(0).Item("SA_Commission").ToString() = "" Then
                                                V_Commission = (ds.Tables(0).Rows(0).Item("SA_Commission").ToString())
                                            End If
                                        End If
                        ...
                        ElseIf VContainCategory.Trim.ToUpper = "NO" And VSlabApplicable.Trim.ToUpper = "With Slab".ToUpper Then
                            ...
                            qryStr = " select * from  " & "BosCenter_DB" & ".dbo.BOS_CommissionSlabwise_SA where (" & Amount & ">=FromAmount and  " & Amount & "<ToAmount) and APIName='" & APIName.Trim & "'; "
